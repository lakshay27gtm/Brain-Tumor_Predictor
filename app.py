...
if uploaded_file is not None:
    ...
    if image is not None:
        ...
        if st.button('Predict'):
            ...
            interpreter.invoke()
            # Get raw output tensor and safely extract the scalar value
            raw_tflite_output = interpreter.get_tensor(output_details[0]['index'])
            confidence = float(raw_tflite_output.squeeze())
